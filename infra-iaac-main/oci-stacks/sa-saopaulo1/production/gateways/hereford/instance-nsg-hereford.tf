output "nsg_id_hereford" {
  value = module.nsg-hereford.nsg_id
}

resource "tls_private_key" "rsa-4096-hereford" {
  algorithm = "RSA"
}

output "private_key_hereford" {
  value     = tls_private_key.rsa-4096-hereford.private_key_pem
  sensitive = true
}

output "public_key_hereford" {
  value     = tls_private_key.rsa-4096-hereford.public_key_openssh
  sensitive = true
}

output "ip_address_hereford" {
...
module "oci_core_instance_hereford" {
  ...
  private_ip                  = "10.1.1.92" //Verificar IP Ñ ESQUECER
  nsg_ids                     = [module.nsg-hereford.nsg_id]
  ssh_public_key              = tls_private_key.rsa-4096-hereford.public_key_openssh
  ...
  volumes = [
    {
      display_name = "bv-hereford-vol1"
      size_in_gbs  = 50
      device       = "/dev/oracleoci/oraclevdb"
    },
    {
      display_name = "bv-hereford-var-lib-docker"
      size_in_gbs  = 50
...
module "nsg-hereford" {
  ...
  nsg_rules = [
    ...
    },
    {
      ...
      protocol    = "6"                      // ICMP ("1"), TCP ("6"), UDP ("17")
      source_type = "NETWORK_SECURITY_GROUP" // or "NETWORK_SECURITY_GROUP"
      source      = module.nsg-pd-lbgw-cs-k1-plus.nsg_id
      min_port    = 9090
      max_port    = 9099
