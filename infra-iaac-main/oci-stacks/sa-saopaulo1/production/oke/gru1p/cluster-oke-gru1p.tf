module "cluster-oke-gru1p" {
  source = "../../../modules/new-oke"

  compartment_id     = var.production
  kubernetes_version = "v1.29.1"
  name               = "cluster-oke-gru1p"
  vcn_id             = module.vcn-oci-oke-gru1p.vcn_id
  type               = "BASIC_CLUSTER"
  ...
  endpoint_config = {
    ...
    subnet_id            = module.vcn-oci-oke-gru1p.subnet_ids["sn-oci-oke-gru1p1"]
  }
  ...
  options = {
    ...
    service_lb_subnet_ids           = [module.vcn-oci-oke-gru1p.subnet_ids["sn-oci-oke-gru1p0"]]
  }
  ...
  node_pools = [
    {
      ...
      placement_configs_subnet_id               = module.vcn-oci-oke-gru1p.subnet_ids["sn-oci-oke-gru1p8"]
      size                                      = 2
      cni_type                                  = "OCI_VCN_IP_NATIVE" //FLANNEL_OVERLAY
      pod_subnet_ids                            = [module.vcn-oci-oke-gru1p.subnet_ids["sn-oci-oke-gru1p8"]]
...
module "nsg-oke-worker" {
  ...
  nsg_rules = [
    ...
    },
    {
      ...
      protocol    = "6"                             // ICMP ("1"), TCP ("6"), UDP ("17")
      source_type = "NETWORK_SECURITY_GROUP"        // or "NETWORK_SECURITY_GROUP"
      source      = module.nsg-nlb-pd-zabbix.nsg_id //module.nsg-qa-lbgw-cs-smartwatch.nsg_id
      min_port    = null
      max_port    = null
...
module "nsg-nlb-pd-zabbix" {
  source         = "../../../modules/new-nsg"
  compartment_id = var.production
  vnc_id         = module.vcn-oci-oke-gru1p.vcn_id
  nsg_name       = "nsg-nlb-pd-zabbix"
  nsg_rules = [
    {
      direction        = "EGRESS"
      protocol         = "all"        // ICMP ("1"), TCP ("6"), UDP ("17") all
