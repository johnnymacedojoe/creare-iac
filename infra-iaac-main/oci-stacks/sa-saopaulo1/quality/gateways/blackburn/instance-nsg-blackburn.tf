output "nsg_id_blackburn" {
  value = module.nsg-blackburn.nsg_id
}

resource "tls_private_key" "rsa-4096-blackburn" {
  algorithm = "RSA"
}

output "private_key_blackburn" {
  value     = tls_private_key.rsa-4096-blackburn.private_key_pem
  sensitive = true
}

output "public_key_blackburn" {
  value     = tls_private_key.rsa-4096-blackburn.public_key_openssh
  sensitive = true
}

output "ip_address_blackburn" {
  value = module.oci_core_instance_blackburn.instance_ip
}

module "oci_core_instance_blackburn" {
  ...
  instance_name               = "blackburn"
  private_ip                  = "10.1.2.91" //Verificar IP Ñ ESQUECER
  nsg_ids                     = [module.nsg-blackburn.nsg_id]
  ssh_public_key              = tls_private_key.rsa-4096-blackburn.public_key_openssh
  ...
  volumes = [
    {
      display_name = "bv-blackburn-vol1"
      size_in_gbs  = 50
      device       = "/dev/oracleoci/oraclevdb"
    },
    {
      display_name = "bv-blackburn-var-lib-docker"
      size_in_gbs  = 50
...
module "nsg-blackburn" {
  source         = "../../../modules/new-nsg"
  compartment_id = var.BU-Safety-compartment_ocid
  vnc_id         = var.vcn-oci-spo1p
  nsg_name       = "nsg-blackburn"
  nsg_rules = [
    {
      direction        = "EGRESS"
    ...
    },
    {
      ...
      protocol    = "6"                      // ICMP ("1"), TCP ("6"), UDP ("17")
      source_type = "NETWORK_SECURITY_GROUP" // or "NETWORK_SECURITY_GROUP"
      source      = module.nsg-qa-lbgw-cs-k4.nsg_id
      min_port    = 9090
      max_port    = 9099
