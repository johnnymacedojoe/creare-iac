module "oci_core_instance_derby" {
  ...
  instance_name               = "derby"
  private_ip                  = "10.1.2.80"
  nsg_ids                     = [module.nsg-derby.nsg_id, var.nsg-spo1p-linux-server]
  ...
  volumes = [
    {
      display_name = "bv-derby-docker-containers"
      size_in_gbs  = 50
      device       = "/dev/oracleoci/oraclevdb"
    },
    {
      display_name = "bv-derby-var-lib-docker"
      size_in_gbs  = 50
...
output "ip_address_derby" {
  value = module.oci_core_instance_derby.instance_ip
}
