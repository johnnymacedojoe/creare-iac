...
resource "oci_network_load_balancer_backend" "backend" {
  for_each = {
    for b in flatten([
      for i, bs in var.backend_sets : [
        for j, be in bs.backends : merge(be, {
          backend_set_name  = bs.name,
          backend_set_index = i,
          backend_index     = j,
          // Construct the name using instance_id and port
          name = "${be.target_id}:${be.port}"
        })
      ]
    ]) : b.name => b
  }
  ...
  name                     = each.value.name // Use dynamic name based on instance ID and port
